...
export function handlePositionStopUpdated(event: PositionStopUpdated): void {

  let position = Position.load(event.params.key.toHexString())

  if (position) {
    position.stopPrice = event.params.stop
    position.save()
  }

}

export function handlePositionTakeUpdated(event: PositionTakeUpdated): void {

  let position = Position.load(event.params.key.toHexString())

  if (position) {
    position.takePrice = event.params.take
    position.save()
  }
...
export function handleNewStopOrder(event: NewStopOrder): void {}

export function handleNewTakeOrder(event: NewTakeOrder): void {}
